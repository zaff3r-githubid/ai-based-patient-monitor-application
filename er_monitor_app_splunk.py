...
def splunk_log(event: Dict[str, Any]):
    ...
    try:
        requests.post(
            ...
            timeout=2,
            verify=False,  # For demos/dev. In production, enable TLS verification.
        )
    except Exception:
        pass
...
with st.sidebar:
    ...
    if st.button("🔄 Reset Monitor / Clear Data", use_container_width=True):
        ...
        st.rerun()
        # --- Splunk observability status (debug / demo aid) ---
    st.caption(f"Splunk HEC URL: {SPLUNK_HEC_URL or 'NOT SET'}")
    st.caption(f"Splunk enabled: {bool(SPLUNK_HEC_URL and SPLUNK_HEC_TOKEN)}")
    st.markdown("### Observability")
    st.caption("Splunk HEC connected" if (SPLUNK_HEC_URL and SPLUNK_HEC_TOKEN) else "Splunk HEC not configured")
# ============================================================================
# LOAD DATA
# ============================================================================
df = None
